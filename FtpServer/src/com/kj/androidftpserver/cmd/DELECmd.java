 ...
 *
 */
public class DELECmd implements Cmd{
	...
	public void dealWithInChan(String info,SelectionKey key){
		...
		if(!success){
			//如果文件不存在,就发回550错误信息.
			ExceptionUtils.sendErrorCommend(key,"DELE");
			return;
		}
		
		//显示创建信息
			MainServerContext.getInstance().addInfo("删除文件: "+filename);
		
		try{
			
		cmdSendBuf.put(("250 File deleted successfully\r\n").getBytes(socketInfo.getEncoding()));
		}catch(Exception e){
			e.printStackTrace();
		}
